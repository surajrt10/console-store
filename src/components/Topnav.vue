<template>
  ...
  <v-toolbar v-if="!isXs" class="px-2" app color="white" elevation="2">
    ...
    <v-btn v-if="isSessionUser" icon class="mx-1" router to="/profile">
      <v-avatar class="mx-2" size="32" color="grey">
        <v-icon>mdi-account-circle</v-icon>
      </v-avatar>
    </v-btn>
        <span v-if="isSessionUser" class="text-grey">Hello, {{props.sessionData.name}}</span>

    <v-btn v-else router to="/login"> Login/Signup </v-btn>

    <v-btn class="mr-1" icon router to="/cart">
